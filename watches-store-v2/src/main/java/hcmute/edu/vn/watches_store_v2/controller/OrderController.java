...
public class OrderController extends ControllerBase {
    ...
    public ResponseEntity<?> createOrder(@RequestBody OrderRequest orderRequest, Principal principal) {
        try {
            ...
            OrderSuccessResponse response = this.orderService.createOrder(orderRequest, findIdByUsername(principal.getName()));

            if (response == null) {
                return response(cartService.getCart(this.findIdByUsername(principal.getName())), HttpStatus.CONFLICT);
            }
